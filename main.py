...
# ---------------------------------------------------------
# Helper Functions
# (ทำงานนอก Prefect context -> ใช้ module-level logger)
# ---------------------------------------------------------

def refresh_session_with_playwright() -> dict:
    """
    ใช้ Playwright intercept XHR request จริงๆ ที่ browser ส่งไป API
    เพื่อดึง headers + cookies ครบชุด ไม่ใช่แค่ cookies จากหน้าเว็บ

    Return: dict ที่มี 'headers' และ 'cookies' ครบชุดพร้อมใช้งาน
    """
...
def extract_items(json_data) -> list:
    ...
    รองรับ 3 formats:
      1. List ตรงๆ                   -> [{"ticker": ...}, ...]
      2. Dict of dicts (format จริง) -> {"data": {"data": {"AAPL": {...}, "MSFT": {...}}}}
      3. List nested ใน dict          -> {"data": {"data": [...]}} หรือ {"data": [...]}
    """
...
# ---------------------------------------------------------
# Prefect Tasks
# (ใช้ get_run_logger() -> log ขึ้น Prefect UI ด้วย)
# ---------------------------------------------------------
...
def fetch_url(config_item: dict, session_data: dict):
    """
    Fetch JSON จาก API
    - Return (json_data, False) ถ้าสำเร็จ
    - Return (None, True)  ถ้าเจอ 403 -> บอก flow ให้ refresh session
    - Return (None, False) ถ้า error อื่นๆ
    - raise Exception ถ้าเจอ 429 -> ให้ Prefect retry พร้อม backoff

    ไม่มีการเรียก Playwright ใน task นี้เด็ดขาด
    ให้ flow เป็นคนจัดการ refresh ครั้งเดียวแล้ว reuse ต่อ
    """
    log = get_run_logger()
    ...
    # retry loop สำหรับ 429 โดยเฉพาะ
    # แยกออกจาก Prefect retry เพราะต้องการ exponential backoff ที่ควบคุมเองได้
    # attempt: 0 -> wait 5s, 1 -> wait 10s, 2 -> wait 20s (max 60s)
...
def main_flow():
    """
    Pipeline: Bronze (raw JSON) + Silver (cleaned Parquet)
    Update frequency: Daily (align กับ stockanalysis.com ที่อัพเดตทุก trading day)

    Session refresh strategy:
    - เปิด Playwright ครั้งเดียวตอนเริ่ม
    - ถ้าเจอ 403 ระหว่าง run -> refresh ครั้งเดียวที่ flow level แล้ว reuse ต่อ
    - ไม่มีการเปิด browser ซ้ำใน task เด็ดขาด
    """
    log = get_run_logger()
    ...
    # track ว่า refresh แล้วหรือยังใน run นี้
    # ถ้า refresh แล้วยังเจอ 403 อีก = ปัญหาฝั่ง server ไม่ใช่ session
...
if __name__ == "__main__":
    ...
    # Scheduler loop: วนรันทุกวันเวลา 08:00 น. (Asia/Bangkok)
    # ทำให้ container ไม่ดับหลัง run แรก และ restart: unless-stopped
    # ใน docker-compose จะ restart ก็ต่อเมื่อ container crash จริงๆ เท่านั้น
    ...
    # รันทันทีตอน start ไม่ต้องรอจนถึงเวลา schedule
    # แก้ปัญหา First-Run Delay: ถ้า deploy ตอนบ่าย 3 โมง
    # จะไม่ต้องรอถึง 8 โมงเช้าวันถัดไปโดยไม่จำเป็น
